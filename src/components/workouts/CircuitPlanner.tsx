...
export default function CircuitPlanner({ sport, onSave, onCancel }: CircuitPlannerProps) {
  // Configuration State - 2026-01-21 19:30 UTC
  const [numCircuits, setNumCircuits] = useState(1);
  const [stationsPerCircuit, setStationsPerCircuit] = useState(4);
  const [seriesMode, setSeriesMode] = useState<'count' | 'time'>('count');
  const [seriesCount, setSeriesCount] = useState(2);
  const [seriesTime, setSeriesTime] = useState(2); // minutes
  const [executionMode, setExecutionMode] = useState<'vertical' | 'horizontal'>('vertical');
  
  // Pause State - 2026-01-21 19:30 UTC
  const [pauseStations, setPauseStations] = useState(10); // seconds
  const [pauseCircuits, setPauseCircuits] = useState(120); // seconds
  const [pauseSeries, setPauseSeries] = useState(120); // seconds
  ...
  const handleRemoveStation = (circuitLetter: string, stationNumber: number) => {
      ...
      for (let seriesNum = 1; seriesNum <= circuit.series; seriesNum++) {
        circuit.stationsBySeries[seriesNum - 1].forEach((station, stationIndex) => {
          ...
          const movelap = {
            ...
            exercise: station.exercise || 'Exercise to be defined',
            reps: station.reps || '',
  ...
  const handleReloadExercises = () => {
    ...
    setCircuits(prevCircuits => {
      ...
      newCircuits.forEach((circuit: any) => {
        circuit.stationsBySeries.forEach((seriesStations: any[]) => {
          seriesStations.forEach((station: any) => {
            if (station.sector) {
              const randomExercise = getRandomExercise(station.sector);
              if (randomExercise) {
    ...
    setActionLog(prev => [...prev, 'Exercises reloaded automatically']);
  };
  ...
  // 2026-01-21 20:15 UTC - Split into two phases: config and table
  // ============================================================================
  
  // Phase 1: Configuration View (matches screenshot)
  if (currentPhase === 'config') {
    return (
      <div className="space-y-4">
        {/* Warning Message - 2026-01-21 20:15 UTC */}
        ...
        {/* Descriptions & Instructions - 2026-01-21 20:15 UTC */}
        <div>
          ...
          <textarea
            ...
            rows={2}
            placeholder="Add circuit description..."
          />
        </div>

        {/* Configuration Grid - 2026-01-21 20:15 UTC */}
        <div className="grid grid-cols-2 gap-6">
          {/* LEFT COLUMN - Circuit Names */}
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-2">
                {Array.from({ length: numCircuits }).map((_, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <span className="font-bold text-gray-700 w-6">{CIRCUIT_LETTERS[idx]}</span>
                    <input
                      type="text"
                      className="flex-1 px-3 py-2 border border-gray-300 rounded bg-yellow-100"
                      placeholder={`Circuit ${CIRCUIT_LETTERS[idx]}`}
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-20 px-3 py-2 border-2 border-blue-500 rounded focus:ring-2 focus:ring-blue-500"
              />
            ...
            <div className="space-y-2">
              ...
              <div className="border rounded p-2">
                {Array.from({ length: stationsPerCircuit }).map((_, idx) => (
                  <div key={idx} className="h-6 bg-yellow-200 border-b border-yellow-400"></div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <input
                  type="radio"
                  checked={seriesMode === 'count'}
                  onChange={() => setSeriesMode('count')}
                  id="series-count"
                />
                <label htmlFor="series-count" className="text-sm">Set series/circuit</label>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="radio"
                  ...
                  id="series-time"
                />
                <label htmlFor="series-time" className="text-sm">Set time/circuit</label>
              </div>
              
              {seriesMode === 'count' ? (
                <div className="flex items-center gap-2">
                  <span className="text-sm">Series/circuit</span>
                  <input
                    type="number"
                    min="1"
                    max="5"
                    ...
                    className="w-20 px-3 py-2 border-2 border-blue-500 rounded"
                  />
                  <button className="p-2 hover:bg-gray-100 rounded">
                    <X size={20} />
                  </button>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setSeriesTime(Math.min(9, Math.max(1, val)));
                    }}
                    className="w-24 px-3 py-2 border-2 border-green-500 rounded text-center"
                  />
                  <span className="text-xs text-gray-500">Disabled when selected Time/circuit</span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={() => setExecutionMode('vertical')}
                  id="exec-vertical"
                />
                <label htmlFor="exec-vertical" className="text-sm">Execution vertically<br/><span className="text-xs text-gray-500">(1 serie for station)</span></label>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="radio"
                  checked={executionMode === 'horizontal'}
                  onChange={() => setExecutionMode('horizontal')}
                  id="exec-horizontal"
                />
                <label htmlFor="exec-horizontal" className="text-sm">Execution orizzontally<br/><span className="text-xs text-gray-500">(all series for station)</span></label>
              </div>
                    ...
                    {Array.from({ length: stationsPerCircuit }).map((_, idx) => (
                      <div key={idx} className="flex gap-1">
                        {Array.from({ length: seriesCount }).map((_, sIdx) => (
                          <div key={sIdx} className="flex-1 h-4 bg-gray-300 border border-gray-400 flex items-center justify-center text-xs">
                            ↔️
                          </div>
                        ))}
                      </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <label className="text-sm w-32">Pause\circuits</label>
                  <select
                    value={pauseCircuits}
                    onChange={(e) => setPauseCircuits(parseInt(e.target.value))}
                    className="flex-1 px-3 py-2 border-2 border-blue-500 rounded"
                  >
                ...
                <div className="flex items-center gap-2">
                  <label className="text-sm w-32">Pause\stations</label>
                  <select
                    value={pauseStations}
                ...
                <div className="flex items-center gap-2">
                  <label className="text-sm w-32">Pause among series</label>
                  <select
                    value={pauseSeries}
                ...
                {seriesMode === 'time' && (
                  <p className="text-xs text-gray-500">Disabled when selected Time/circuit</p>
                )}

                <div className="border rounded p-2">
                  {Array.from({ length: stationsPerCircuit }).map((_, idx) => (
                    <div key={idx} className="flex items-center gap-1 mb-1">
                      <div className="w-6 h-6 rounded-full bg-red-400 border-2 border-red-600 flex items-center justify-center text-xs font-bold">
                        {idx + 1}
                      </div>
                    </div>
                  ))}
        ...
        {/* Action Buttons - 2026-01-21 20:15 UTC */}
        <div className="flex justify-between items-center pt-4 border-t">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-6 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-800 font-medium"
            >
              Create circuit
            </button>
          </div>
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-300 rounded-lg">
        <div className="bg-gray-100 px-4 py-3 border-b border-gray-300 flex items-center justify-between">
          ...
          <button
            onClick={() => setShowPreferencesModal(true)}
            className="flex items-center gap-2 px-3 py-1.5 bg-purple-500 text-white rounded-md hover:bg-purple-600 text-sm"
          >
            <Settings size={16} />
            Preferences
          </button>
        ...
        {/* Table with X buttons on the left - 2026-01-21 21:00 UTC */}
        <div className="flex">
          {/* Left side - X buttons column - 2026-01-21 21:00 UTC */}
          {/* X inside circle design */}
          <div className="relative flex flex-col pt-[72px]" style={{width: '45px'}}>
            {circuits.map((circuit, circuitIndex) => {
              // 2026-01-22 12:15 UTC - Use first series to get stations count (all series have same count)
              const stationsPerSeries = circuit.stationsBySeries[0]?.length || 0;
              const rowHeight = 36; // px
              
              return (
                <React.Fragment key={`x-buttons-${circuit.letter}-${circuitIndex}`}>
                  {/* 2026-01-21 21:45 UTC - X buttons: Big at circuit start, Small at EACH series start */}
                  
                  {/* Render for each series */}
                  {Array.from({ length: circuit.series }).map((_, seriesIdx) => {
                    const isFirstSeries = seriesIdx === 0;
                    const seriesNumber = seriesIdx + 1;
                    
                    return (
                      <React.Fragment key={`series-${circuit.letter}-${seriesNumber}`}>
                        {/* First row of this series - contains X button(s) */}
                        <div 
                          className="relative flex items-center justify-center gap-1"
                          style={{height: `${rowHeight}px`}}
                        >
                          {/* Big Red X - only on first series (circuit start) */}
                          {isFirstSeries && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              className="w-8 h-8 rounded-full border-2 border-red-600 flex items-center justify-center hover:bg-red-50 transition-colors"
                          ...
                          {/* Small X - on EVERY series */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              removeSeries(circuit.letter, seriesNumber);
                            }}
                            className="w-6 h-6 rounded-full border-2 border-gray-700 flex items-center justify-center hover:bg-gray-100 transition-colors"
                            title={`Delete series ${seriesNumber} of circuit ${circuit.letter}`}
                          >
                            <X size={14} className="text-gray-700" strokeWidth={2.5} />
                          </button>
                        </div>
                        
                        {/* Fill remaining rows of this series (stations 2, 3, 4, ...) */}
                        {stationsPerSeries > 1 && (
                          <div style={{height: `${rowHeight * (stationsPerSeries - 1)}px`}} />
                        )}
                      </React.Fragment>
                    );
                  })}
           ...
           {/* Right side - Table - 2026-01-22 10:10 UTC - Circuit checkbox under letter */}
           ...
           <table className="w-full border-collapse text-sm">
             <thead>
               <tr className="bg-gray-50">
                 ...
                 <th colSpan={2} className="border border-gray-300 px-3 py-2 text-sm font-semibold text-left">
                   Load of work
                 </th>
               </tr>
            ...
            <tbody>
              {circuits.map((circuit, circuitIdx) => {
                ...
                const totalRows = circuit.series * (circuit.stationsBySeries[0]?.length || 0);
                let rowIndex = 0;
                        ...
                        rowIndex++;
                         ...
                         return (
                           <tr key={`${circuit.letter}-${seriesIdx}-${stationIdx}`} className="hover:bg-gray-50" style={{height: '36px'}}>
                             {/* Circuit Letter with Checkbox underneath - 2026-01-22 10:10 UTC */}
                             {isFirstRowOfCircuit && (
                               <td 
                                 rowSpan={totalRows} 
                             ...
                             {isFirstRowOfSeries && (
                               <td 
                                 rowSpan={circuit.stationsBySeries[0]?.length || 0}
                            ...
                            {/* Exercise with Three Bars Menu - 2026-01-21 20:30 UTC */}
                            {/* 2026-01-22 12:20 UTC - Removed z-0 to fix dropdown stacking context */}
                            <td className="border border-gray-300 px-0 py-0 bg-green-50">
                              <div className="flex items-center gap-1 exercise-menu-container relative z-[10000]">
                                <input
                                  type="text"
                                  value={station.exercise}
                                  onClick={(e) => {
                                    // 2026-01-22 13:40 UTC - Fixed to include x,y coordinates
                                    e.stopPropagation();
                                  ...
                                  className="flex-1 px-2 py-1 text-xs border-0 bg-green-100 cursor-pointer hover:bg-green-200"
                                />
                                <button
                                  onClick={(e) => {
                                    // 2026-01-22 12:30 UTC - Calculate position for dropdown
                                    // 2026-01-22 13:45 UTC - Added debugging
                                    e.stopPropagation();
                                    const rect = e.currentTarget.getBoundingClientRect();
                                    const menuData = {
                                      circuit: circuit.letter, 
                                      ...
                                      station: station.stationNumber,
                                      x: rect.left,
                                      y: rect.bottom + 4 // 4px below the button
                                    };
                                    console.log('Opening exercise menu:', menuData);
                                    setShowExerciseMenu(menuData);
                                  }}
      ...
      {showSectorSelector && (selectedCircuitForSector || selectedStationForSector) && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-6xl w-full max-h-[90vh] flex flex-col">
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 p-4 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
      ...
      <div className="flex items-center gap-3 mt-6 border-t pt-6">
       ...
       {/* Preview Section - 2026-01-21 22:10 UTC */}
       <div className="mt-4 p-4 bg-gray-50 border border-gray-300 rounded">
         ...
         <div className="text-xs text-gray-500 italic mt-2">
           This is the PREVIEW displayed at the bottom
         </div>
       </div>
       
       {/* Action Buttons - 2026-01-22 10:25 UTC - Reload exercises on left, Cancel/Generate on right */}
       <div className="flex justify-between items-center pt-4 border-t">
         {/* Left side - Reload exercises button */}
         <div className="flex items-center gap-2">
           <button
         ...
         {/* Right side - Cancel and Generate Circuit buttons */}
         <div className="flex gap-3">
           <button
             onClick={onCancel}
             className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 font-medium"
    ...
    {showExerciseMenu && typeof document !== 'undefined' && ReactDOM.createPortal(
      <>
        ...
        />
        <div 
          className="bg-white border border-gray-400 rounded-lg shadow-2xl w-64"
          style={{ 
            position: 'fixed',
            zIndex: 99999,
        ...
        <button
          onClick={() => {
            ...
            if (circuit) {
              handleRemoveStation(showExerciseMenu.circuit, showExerciseMenu.station);
            }
            setShowExerciseMenu(null);
          }}
          className="w-full px-4 py-2 text-left text-sm hover:bg-red-50 text-red-600 border-b border-gray-200"
        >
          Remove station
        </button>
        <button
          ...
          className="w-full px-4 py-2 text-left text-sm hover:bg-blue-50 border-b border-gray-200"
        >
          Select exercise + rip
        </button>
        <button
          onClick={() => {
            ...
            if (circuit && circuit.stationsBySeries[seriesIdx]) {
              ...
              if (currentSector) {
                const randomExercise = getRandomExercise(currentSector);
                ...
                if (randomExercise) {
                  ...
                  alert(`No exercises available for ${currentSector}. Please assign a sector first.`);
                }
              } else {
